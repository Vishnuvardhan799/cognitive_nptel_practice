...
data = []
# Match each Unit block
units = re.findall(r'### \*\*Unit \d+ \(Assessment (\d+)\)\*\*(.*?)(?=### \*\*Unit|---|$)', text, re.DOTALL)

for assessment_num_str, unit_content in units:
    assessment_num = int(assessment_num_str)
    
    # Match each Question block within the unit
    # Question starts with * [cite_start]**Question X:** or * **Question X:**
    questions = re.findall(r'\* (?:\[cite_start\])?\*\*Question (\d+):\*\*(.*?)(?=\* (?:\[cite_start\])?\*\*Question|$)', unit_content, re.DOTALL)
    
    for q_num_str, q_content in questions:
        q_num = int(q_num_str)
        
        # Split q_content into parts: Question, Options, Accepted Answer
        parts = re.split(r'\* (?:\[cite_start\])?\*\*(Options|Accepted Answer):\*\*', q_content)
        ...
        for i in range(1, len(parts), 2):
            ...
            val = parts[i+1].strip()
            if label == "Options":
                # Split options by [cite: X] followed by comma, semicolon, or line break
                # Some options are separated by semicolons
                raw_options = re.split(r'\[cite: \d+\](?:\[cite_start\])?(?:[,;]|\s+|$)', val)
                for opt in raw_options:
                    # Filter out noise like [cite_start] or (Note: ...)
                    opt_clean = re.sub(r'\[cite_start\]', '', opt).strip()
                    opt_clean = re.sub(r'\(Note:.*?\)', '', opt_clean).strip()
                    if opt_clean:
                        options.append(opt_clean)
            elif label == "Accepted Answer":
                answer_text = re.sub(r'\[cite: \d+\]', '', val).strip()
                answer_text = re.sub(r'_$', '', answer_text).strip()
        
        # Clean question text
        q_text = re.sub(r'\[cite_start\]', '', q_text)
        q_text = re.sub(r'\[cite: \d+(?:, \d+)*\]', '', q_text).strip()
        ...
        # Handle special cases where options are obscured or missing
        # We'll use the user-provided answer and add distractor placeholders if needed
        # Or just use the one option if that's all we have.
        
        # Manual patches for common missing data in 2018
        if assessment_num == 1 and q_num == 3:
            options = ["behaviorism", "structuralism", "functionalism", "gestalt"]
            correct_index = 0
        elif assessment_num == 2 and q_num == 3:
            options = ["Functionalism", "Gestalt", "Structuralism", "Behaviorism"]
            correct_index = 1
        elif assessment_num == 3 and q_num == 3:
            options = ["tasks that require resource-limited processing", "tasks that require data-limited processing", "automatic processing", "controlled processing"]
            correct_index = 0
        elif assessment_num == 4 and q_num == 3:
            options = ["decay", "interference", "both decay and interference", "neither decay nor interference"]
            correct_index = 2
        elif assessment_num == 5 and q_num == 3:
            options = ["Semantic memory, episodic memory", "Episodic memory, semantic memory", "Short-term, long-term", "Procedural, declarative"]
            correct_index = 0
        elif assessment_num == 6 and q_num == 3:
            options = ["spreading activation", "cognitive economy", "typicality effect", "hierarchical network"]
            correct_index = 0
        elif assessment_num == 7 and q_num == 3:
            options = ["Exemplar view", "Prototype view", "Classical view", "Knowledge-based view"]
            correct_index = 0
        elif assessment_num == 8 and q_num == 3:
            options = ["Verbal and Imagery", "Acoustic and Visual", "Semantic and Episodic", "Procedural and Declarative"]
            correct_index = 0
        elif assessment_num == 9 and q_num == 3:
            options = ["Morpheme", "Phoneme", "Lexeme", "Syntax"]
            correct_index = 0
        elif assessment_num == 10 and q_num == 3:
            options = ["Associative learning approach", "Insight approach", "Information processing approach", "Heuristic approach"]
            correct_index = 0
        elif assessment_num == 11 and q_num == 3:
             q_text = "Which of the reasoning can explain the following: All birds have wings. A robin is a bird. Therefore, a robin has wings."
             options = ["Inductive reasoning", "Deductive reasoning", "Syllogistic reasoning", "Conditional reasoning"]
             correct_index = 0
        elif assessment_num == 12 and q_num == 3:
            options = ["Expected utility theory", "Prospect theory", "Multi-attribute utility theory", "Sunk cost effect"]
            correct_index = 1

        # If options are still empty or index not found, and we have an answer, create a single-option list
